...
labevents_subset = open_dataset("~/Desktop/labevents_pq") %>%
  to_duckdb() %>%
...
```
```{r, warning=FALSE}
merged = left_join(labevents_subset, icustays_tble,
                   by = "subject_id")
```
...
```{r}
labevents_tble = adjusted_labevents %>% 
...
chartevents_subset = open_dataset("~/Desktop/chartevents_pq") %>%
  to_duckdb() %>%
...
```{r}
adjusted_chartevents = chartevents_subset %>% 
  group_by(subject_id, stay_id, itemid) %>%
  slice_min(storetime) %>%
  select(subject_id, stay_id, itemid, valuenum)
```
...
```{r}
final_chartevents = adjusted_chartevents %>% 
  group_by(subject_id, stay_id, itemid) %>% 
...
```{r}
chartevents_tble = final_chartevents %>% 
  pivot_wider(names_from = itemid, values_from = valuenum)
```
